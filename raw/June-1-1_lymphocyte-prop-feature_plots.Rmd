---
title: "June-1-1_T-cell_marker_plots"
...
Goal:
1) make updated proportion plots for T cells
2) make additional marker plots for T cells
3) make proportion and marker plots for lymphocytes
...
### overview whole plot for lymphocytes
...
```{r}
maskTable <- generateMask(dims=lymphocytes@reductions$umap@cell.embeddings, 
                          clusters=lymphocytes$annotation_V2)
data <- data.table(lymphocytes@reductions$umap@cell.embeddings, 
                   cluster=lymphocytes$annotation_V2,
                   lymphocytes@meta.data)
centers <- data %>% 
  dplyr::group_by(cluster) %>% 
  dplyr::summarize(umap_1 = median(umap_1), umap_2 = median(umap_2)) 
...
g <- ggplot(data, aes(x=umap_1, y=umap_2)) + 
  ...
  guides(color = guide_legend(override.aes = list(size = 3))) 
g
ggsave(filename = paste0(outdir,"lymphocytes_V1p4_ann-V2_umap_split_mascarade.pdf"), plot = g, width = 18, height = 7)
...
g <- ggplot(data, aes(x=umap_1, y=umap_2)) + 
  ...
  guides(color = guide_legend(override.aes = list(size = 3))) 
g
ggsave(filename = paste0(outdir,"lymphocytes_V1p4_ann-V2_umap_mascarade.pdf"), plot = g, width = 18, height = 7)
```

```{r}
p1 <- DotPlot(lymphocytes, group.by = "annotation_V2",features = c("Cd19","Kit","Gata3","Ncr1","Tyrobp","Cd3e","Rorc","Klrb1c","Cd4","Foxp3","Il4","Il5","Il13","Cd8a","Lef1","Ccr7","Slamf6","Ccr4","Cd69","Il2ra","Cd44","Prf1","Gzmb","Gzmk","Lag3","Pdcd1","Tox","Havcr2","Mki67"))+scale_color_gradientn(colors = c("grey90","red2","red3"))+ RotatedAxis()+coord_flip()
ggsave(filename = paste0(outdir,"Supp_lymphocytes_Dotplot_annotation-V2.pdf"), plot = p1, units = "cm",height = 24,width = 20)
```

```{r}
markers <- c("Cd3e","Ncr1","Tyrobp","Klrb1c","Kit","Cd4","Foxp3","Gata2","Il4","Il5","Il13","Cd8a","Lef1","Ccr7","Slamf6","Ccr4","Cd69","Il2ra","Cd44","Prf1","Gzmb","Gzmk","Lag3","Pdcd1","Tox","Havcr2","Mki67")
...
ggsave(paste0(outdir, "lymphocytes_markers_featurePlot.pdf"), plot = g, width = 30, height = 30)


```


### ann-V2 proportion plots
...
```{r}
clust_prop <- lymphocytes@meta.data %>% group_by(annotation_V2,compartment,orig.ident) %>% summarise(n_subpop = n()) %>% ungroup()
clust_prop <- left_join(clust_prop,sample_imm_size, by = "orig.ident")
clust_prop <- clust_prop %>% group_by(orig.ident) %>% mutate(percentage = 100*n_subpop/n_immune)
```

```{r}
ratio <- clust_prop %>% group_by(annotation_V2) %>% reframe(FC = log2(percentage[orig.ident == "KO_HC"]/percentage[orig.ident == "WT_HC"]))
```


```{r}
g3 <- ggplot(clust_prop,aes(x = factor(orig.ident, levels = c("WT_HC", "KO_HC")), y = percentage, fill = annotation_V2))+
  geom_bar(aes(color = orig.ident), fill = "white",stat = "identity", position = "dodge")+
  scale_color_manual("condition",values = c("KO_HC" = "darkblue", "WT_HC" = "red3"))+theme_classic()+
  ylab("% in immune cells")+
  xlab("conditions")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  facet_wrap(~annotation_V2, scales = "free")
g3  
ggsave(filename = paste0(outdir,"lymphocyte_percentage_bar_fill_by_condition_ann-V2.pdf"), plot = g3, width = 8, height = 8)


  
g4 <- ggplot(ratio, aes( x = reorder(annotation_V2, -FC), y = FC))+
  geom_bar(aes(fill = annotation_V2),stat = "identity", position = "dodge")+
  scale_fill_manual(values = pal2)+theme_classic()+
  ylab("log2 % in KO v.s. WT immune cells")+
  xlab("subpopulations")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  geom_hline(yintercept = c(-1,1), color = "red3", linetype = 3)
g4
ggsave(filename = paste0(outdir,"lymphocyte_log2_percentage_bar_ann-V2.pdf"), plot = g4, width = 6, height = 4)
```
## T cells
...
```{r}
T_cells <- AddModuleScore(T_cells, features = list(Tr1_genes),name = "Tr1_signature_Houssain")
```

```{r}
FeaturePlot(T_cells, features = "Tr1_signature_Houssain1")+scale_color_gradientn(colors = c("grey","white","red3"))
```
...
#### cell type state markers featureplot
...
```{r}
VlnPlot(T_cells,features = "Itgal", group.by = "annotation_V1", split.by = "orig.ident")
```


```{r}
g20 <- ggplot(data, aes(x=umap_1, y=umap_2)) + 
  geom_point(aes(color=Tr1_signature_Houssain1), size = 0.5)+
    geom_path(data=maskTable, aes(group=group),color = "grey60") +
  ggrepel::geom_text_repel(data = centers, aes(label=ifelse(cluster %in% c("c2: Treg","c4: Tem/ex"), cluster, "")), size = 3.5, color = "black", nudge_x = 1, nudge_y = 2) +
    scale_color_gradientn(colors = c("grey50", "grey90","red3"))+
    coord_fixed() + 
    theme_classic()+
  ggtitle("Tr1 Signature Sultan 2024")+
  theme(legend.position = "right")

g20
ggsave(paste0(outdir, "T_Tr1_mascarade_ann-V1.pdf"),plot = g20, width = 6, height = 8 )
```
...
```{r}
maskTable <- generateMask(dims=T_cells@reductions$umap@cell.embeddings, 
                          clusters=T_cells$annotation_V2)
data <- data.table(T_cells@reductions$umap@cell.embeddings, 
                   cluster=T_cells$annotation_V2,
                   T_cells@meta.data)
centers <- data %>% 
  dplyr::group_by(cluster) %>% 
  dplyr::summarize(umap_1 = median(umap_1), umap_2 = median(umap_2)) 
...
g <- ggplot(data, aes(x=umap_1, y=umap_2)) + 
  ...
  guides(color = guide_legend(override.aes = list(size = 3))) 
g
ggsave(filename = paste0(outdir,"T_V1p4_ann-V2_umap_split_mascarade.pdf"), plot = g, width = 18, height = 7)
...
g <- ggplot(data, aes(x=umap_1, y=umap_2)) + 
  ...
  guides(color = guide_legend(override.aes = list(size = 3))) 
g
ggsave(filename = paste0(outdir,"T_V1p4_ann-V2_umap_mascarade.pdf"), plot = g, width = 18, height = 7)
```
...
```{r}
VlnPlot(subset(T_cells), group.by = "annotation_V1", features = "Slamf6", split.by = "orig.ident")+scale_fill_manual(values = c("WT_HC"="red3", "KO_HC" = "darkblue"))
VlnPlot(T_cells, group.by = "annotation_V1", features = "Tcf7", split.by = "orig.ident")+scale_fill_manual(values = c("WT_HC"="red3", "KO_HC" = "darkblue"))
VlnPlot(T_cells, group.by = "annotation_V1", features = "Lef1", split.by = "orig.ident")+scale_fill_manual(values = c("WT_HC"="red3", "KO_HC" = "darkblue"))
VlnPlot(T_cells, group.by = "annotation_V1", features = "Xcl1", split.by = "orig.ident")+scale_fill_manual(values = c("WT_HC"="red3", "KO_HC" = "darkblue"))
VlnPlot(T_cells, group.by = "annotation_V1", features = "Id3", split.by = "orig.ident")+scale_fill_manual(values = c("WT_HC"="red3", "KO_HC" = "darkblue"))
VlnPlot(T_cells, group.by = "annotation_V1", features = "Pdcd1", split.by = "orig.ident")+scale_fill_manual(values = c("WT_HC"="red3", "KO_HC" = "darkblue"))
```
